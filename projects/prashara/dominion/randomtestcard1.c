...
int main(int argc, char** argv) {
	...
	for (i = 0; i < NUM_TEST; i++) {
		...
		if (G.handCount[0] != (handStart + 2)) {
			printf("\nFailed to add 3 cards to hand.\n");
			handFailure++;
			result = 0;
		}

		if (G.deckCount[0] != (deckStart - 3)) {
			printf("\nFailed to remove 3 cards from deck.\n");
			deckFailure++;
			result = 0;
		}

		if (G.playedCardCount != (discardStart + 1)) {
			printf("\nFailed to discard smithy card from hand.\n");
			discardFailure++;
			result = 0;
		}
	...
	printf("\nTotal numbers of Tests of discarding smithy failed: %d\n",discardFailure);	
	return 0;
}
