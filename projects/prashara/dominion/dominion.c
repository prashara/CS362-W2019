...
int play_adventurer(int currentPlayer, struct gameState *state, int drawntreasure, int temphand[],int z) {
	int cardDrawn;
...
int play_smithy( int currentPlayer, struct gameState *state, int handPos) {
	//+3 Cards
	for (int i = 0; i <= 3; i++)         // Error: Instead of 3 we are drawing 4 cards  due to <= condition.
	{
		drawCard(currentPlayer, state);
	}
...
int play_remodel(int currentPlayer,  struct gameState *state , int handPos, int choice1, int choice2) {
	int j = state->hand[currentPlayer][choice1];  //store card we will trash

	if ((getCost(state->hand[currentPlayer][choice1]) + 2) < getCost(choice2)) // Error: > sign is replaced by < . It will cause incorrect behaviour.
	{
		return -1;
	}
	...
	for (int i = 0; i <= state->handCount[currentPlayer]; i++)        //  Error: Invalid bounds i should be < instead of <= it will cause segmentation faults.
	{
...
int play_councilroom(int currentPlayer, struct gameState *state,int handPos) {
	//+4 Cards
	for (int i = 0; i < 4; i++)
	{
		drawCard(currentPlayer, state);
	...
	for (int i = 0; i <= state->numPlayers; i++)       // Error: <= used instead of <. incorrect bounds it can cause segmentation fault.
	{
		if (i != currentPlayer)
		{
			drawCard(i, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      // call adventurer function here.
		
		return play_adventurer(currentPlayer, state, drawntreasure, temphand, z);

    case council_room:
		return play_councilroom(currentPlayer, state, handPos);
    ...
    case remodel:
		// call play_remodel function here.
		return play_remodel(currentPlayer, state, handPos, choice1, choice2);

    case smithy:
		// call play_smithy function here.
		return play_smithy(currentPlayer, state, handPos);
		
    case village:
		// call play_village function here.
